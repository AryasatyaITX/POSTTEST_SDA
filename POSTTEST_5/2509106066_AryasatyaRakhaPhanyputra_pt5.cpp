...
void tampilkanStokSembako(BarangNode* head) {
 // --- LENGKAPI KODE DI SINI ---
    if(head == nullptr) { // kondisi ketika linked list kosong atau tidak memiliki node
        cout << "Gudang kosong." << endl;
        return; //menghentikan fungsinya
    }
    BarangNode* temp = head; // melakukan traversal dimulai dari head pada linked list
    do {
        cout << " - " << temp ->namaBarang << " : " << temp ->stok << endl; //Menampilkan data node
        temp = temp ->next; // pindah ke node selanjutnya
    }
        while(temp != head); // berhenti ketika saat melakukan traversal kembali ke head untuk menghindari infinite loop
}
